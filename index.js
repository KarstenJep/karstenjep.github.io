...
function move() {
  ...
  rocket.style.bottom = 50 + incrementer * .25 + '%'
  rocket.style.left = 80 + incrementer * 0.05 + '%'
  airplane.style.bottom = 80 + incrementer * .00 + '%'
  airplane.style.left = -30 + incrementer * 0.3 + '%'
  pg.style.bottom = 75 + incrementer * -.05 + '%'
  pg.style.left = 72 + incrementer * -0.3 + '%'
  html.style.left = 10 + incrementer * -.05 + '%'
  html.style.bottom = 57 + incrementer * -0.05 + '%'
  css.style.left = 19 + incrementer * -.05 + '%'
  css.style.bottom = 56 + incrementer * -0.08 + '%'
  javascript.style.left = 31 + incrementer * -.05 + '%'
  javascript.style.bottom = 57 + incrementer * -0.1 + '%'
  react.style.left = 42 + incrementer * -.05 + '%'
  react.style.bottom = 57 + incrementer * -0.12 + '%'
  node.style.left = 53 + incrementer * .05 + '%'
  node.style.bottom = 57 + incrementer * -0.12 + '%'
  postgresql.style.left = 64 + incrementer * .05 + '%'
  postgresql.style.bottom = 57 + incrementer * -0.1 + '%'
  python.style.left = 75 + incrementer * .05 + '%'
  python.style.bottom = 57 + incrementer * -0.08 + '%'
  git.style.left = 86 + incrementer * .05 + '%'
  git.style.bottom = 57 + incrementer * -0.05 + '%'
